...
export class HandleCmdBuilder<UICtx extends BaseUIContext> extends AbstractCmdHandler<UICtx> {
    ...
    private async handleBuildProcess(userId: string, text: string, ctx: UICtx, builder: CommandBuilder, dispatcher: CmdDispatcher<UICtx>): Promise<ICmdHandlerResponce|void> {
        if (builder.isUserOnBuild(userId)) {
            ...
            if (stepRes.IsCompiled) {
                ...
                return await dispatcher.CommandInvoker.invoke(userId, stepRes.Result, ctx)
            }
