...
export class TelegramUI extends BaseUI<TgContext> {
    ...
    protected async sendMessageImpl(user_id: string, message: string, mk_opts?: IMarkupOption[]): Promise<string> {
        const rmk = mk_opts ? {
            reply_markup: {
    ...
    protected async editMessageImpl(user_id: string, message_id: string, message: string, mk_opts?: IMarkupOption[]): Promise<void> {
        const rmk = mk_opts ? {
            reply_markup: {
                inline_keyboard: this.createCommonKeyboard(mk_opts)
            }
